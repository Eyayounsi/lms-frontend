...
// src/app/shared/service/auth/auth.interceptor.ts
...
// src/app/shared/service/auth/blocked.interceptor.ts
var isLoggingOut = false;
var blockedInterceptor = (req, next) => {
  const blockedService = inject(BlockedService);
  const router = inject(Router);
  return next(req).pipe(catchError((error) => {
    if (error.status === 423 && !req.url.includes("/auth/logout")) {
      const message = error.error?.message || "Votre compte a \xE9t\xE9 bloqu\xE9 par l'administrateur.";
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(AppComponent, [{
    type: Component,
    args: [{ selector: "app-root", imports: [RouterOutlet, CommonModule], template: "<router-outlet></router-outlet>\n<title>{{ page | titlecase }} | CRMS - Advanced Bootstrap 5 Admin Template for Customer Management</title>" }]
  }], () => [{ type: Router }], null);
})();
(() => {
